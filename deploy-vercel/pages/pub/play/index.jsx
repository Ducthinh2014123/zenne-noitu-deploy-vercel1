...
import { IconGamepad, IconLock, IconLogIn, IconSparkles, IconGlobe, IconClock, IconRocket, IconLink, IconAlertTriangle, IconBookOpen, IconTrophy } from '../../../components/icons';
    ...
    <PubLayout title="Chơi Nối Từ Online">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-10">
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-6">
          <h2 className="flex items-center gap-2 text-lg font-bold text-white mb-4"><IconSparkles className="w-4 h-4 text-indigo-400" /> Tạo Phòng Mới</h2>
          <div className="space-y-4">
            <div>
              <label className="flex items-center gap-1.5 text-sm text-gray-400 mb-1"><IconGlobe className="w-3.5 h-3.5" /> Ngôn ngữ</label>
            ...
            <div>
              <label className="flex items-center gap-1.5 text-sm text-gray-400 mb-1"><IconClock className="w-3.5 h-3.5" /> Mỗi lượt: <strong className="text-white">{sec}s</strong></label>
            ...
            <button onClick={create} disabled={loading}
              className="w-full flex items-center justify-center gap-2 py-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white font-semibold rounded-lg transition-colors">
              {loading ? 'Đang tạo...' : (<><IconRocket className="w-4 h-4" /> Tạo phòng & Vào</>)}
            </button>
          </div>
        </div>
        ...
        {/* Join */}
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-6">
          <h2 className="flex items-center gap-2 text-lg font-bold text-white mb-4"><IconLink className="w-4 h-4 text-indigo-400" /> Tham Gia Phòng Có Sẵn</h2>
          <form onSubmit={join} className="space-y-4">
            ...
            <button type="submit" disabled={joinId.trim().length < 4}
              className="w-full flex items-center justify-center gap-2 py-3 bg-green-700 hover:bg-green-600 disabled:opacity-40 text-white font-semibold rounded-lg transition-colors">
              <IconLogIn className="w-4 h-4" /> Vào phòng
            </button>
          </form>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <h3 className="flex items-center gap-2 font-bold text-white mb-3"><IconBookOpen className="w-4 h-4 text-indigo-400" /> Cách chơi</h3>
        <ol className="space-y-2 text-sm text-gray-400 list-decimal list-inside">
          ...
          <li className="flex items-start gap-1.5"><IconTrophy className="w-3.5 h-3.5 mt-0.5 flex-shrink-0 text-yellow-500" /> Người còn lại cuối cùng / nhiều điểm nhất thắng!</li>
        </ol>
      </div>
    </PubLayout>
  );
