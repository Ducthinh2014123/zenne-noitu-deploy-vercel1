...
const Input = ({ label, icon, error, ...props }) => (
  <div>
    ...
    <div className="relative">
      ...
      <input {...props} className={`w-full bg-gray-800 border rounded-xl ${icon?'pl-10':'pl-4'} pr-4 py-3 text-white placeholder-gray-600 focus:outline-none transition-colors text-sm ${error?'border-red-600 focus:border-red-500':'border-gray-700 focus:border-indigo-500'}`}/>
    </div>
...
const PwStrength = ({ pw }) => {
  const n = [/.{8,}/,/[A-Z]/,/[0-9]/,/[^A-Za-z0-9]/].filter(r=>r.test(pw)).length;
  const c = ['','bg-red-500','bg-orange-400','bg-yellow-400','bg-green-500'][n]||'bg-gray-700';
  if (!pw) return null;
  return <div className="flex gap-1 mt-1.5">{[0,1,2,3].map(i=><div key={i} className={`h-1 flex-1 rounded-full ${i<n?c:'bg-gray-700'}`}/>)}<span className="text-xs text-gray-500 ml-1">{['','Yếu','TB','Tốt','Mạnh'][n]}</span></div>;
};
...
export default function AuthLogin() {
  ...
  useEffect(() => {
    setAUrl(localStorage.getItem('nt_api_url') || '');
    setAKey(localStorage.getItem('nt_api_key') || '');
  }, []);
  ...
  useEffect(() => {
    const m = { OAuthSignin:'Lỗi OAuth.', OAuthCallback:'Lỗi callback OAuth.',
      OAuthAccountNotLinked:'Email đã dùng với phương thức khác.',
      CredentialsSignin:'Email hoặc mật khẩu không đúng.', Default:'Đã xảy ra lỗi.' };
  ...
  const doLogin = async (e) => {
    e.preventDefault();
    const e2 = {};
    if (!lEmail) e2.email = 'Nhập email';
    if (!lPw)    e2.pw    = 'Nhập mật khẩu';
    if (Object.keys(e2).length) { setLErr(e2); return; }
    setLErr({}); setLoading('creds'); setMsg({type:'',text:''});
    const res = await signIn('credentials', { email:lEmail, password:lPw, redirect:false });
    setLoading('');
    if (res?.error) setMsg({ type:'error', text:'Email hoặc mật khẩu không đúng.' });
  };

  const doRegister = async (e) => {
    e.preventDefault();
    const e2 = {};
    if (!rName||rName.length<3) e2.name  = 'Tên tối thiểu 3 ký tự';
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(rEmail)) e2.email = 'Email không hợp lệ';
    if (!rPw||rPw.length<6)    e2.pw    = 'Mật khẩu tối thiểu 6 ký tự';
    if (rPw!==rPw2)            e2.pw2   = 'Mật khẩu không khớp';
    if (Object.keys(e2).length) { setRErr(e2); return; }
  ...
  const doAdminLogin = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('nt_api_key',aKey.trim());
      router.push('/pending');
    } catch { setAErr('Không kết nối được đến API.'); }
    setALoading(false);
  };
          ...
          <div className="bg-gray-900/90 border border-gray-800 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
            ...
            <div className="flex border-b border-gray-800">
              {[['login','🔑 Đăng nhập'],['register','✨ Đăng ký']].map(([k,l])=>(
                <button key={k} onClick={()=>{setTab(k);setMsg({type:'',text:''});}} className={`flex-1 py-4 text-sm font-semibold transition-colors border-b-2 ${
                  tab===k?'text-white border-indigo-500 bg-indigo-500/5':'text-gray-500 border-transparent hover:text-gray-300'
                }`}>{l}</button>
              ))}
            </div>
            ...
            <div className="p-6">
              ...
              {msg.text && (
                ...
                }`}><span className="mt-0.5">{msg.type==='success'?'✅':'⚠️'}</span><span>{msg.text}</span></div>
              )}
              ...
              {tab==='login' && (
                <>
                  ...
                  <p className="text-center text-sm text-gray-500 mt-5">Chưa có tài khoản?{' '}
                    <button onClick={()=>setTab('register')} className="text-indigo-400 hover:text-indigo-300 font-medium">Đăng ký ngay →</button>
                  </p>
                </>
              )}
                  ...
                  <div className="grid grid-cols-4 gap-2 mb-1">
                    {[['google',<GoogleIcon/>],['github',<GithubIcon/>],['facebook',<FacebookIcon/>],['microsoft',<MicrosoftIcon/>]].map(([p,ic])=>(
                      <button key={p} onClick={()=>oAuth(p)} disabled={any} title={'Sign in with '+p}
                        className="flex items-center justify-center py-2.5 bg-gray-800 border border-gray-700 rounded-xl hover:border-gray-500 hover:bg-gray-700 transition-colors disabled:opacity-50">{ic}</button>
                    ))}
                  </div>
              ...
              {/* ══ ADMIN API KEY (collapsible) ══ */}
              <div className="mt-6 border-t border-gray-800 pt-5">
                <button onClick={()=>setShowAdmin(s=>!s)}
                  className="w-full flex items-center justify-between text-xs text-gray-500 hover:text-gray-400 group">
                  <span className="flex items-center gap-1.5"><span>🔐</span> Admin — đăng nhập bằng API Key</span>
                ...
                {showAdmin && (
                  <form onSubmit={doAdminLogin} className="mt-4 space-y-3">
                    {aErr && <div className="p-2.5 bg-red-900/30 border border-red-700/50 rounded-lg text-red-400 text-xs">{aErr}</div>}
                    <Input label="API URL" type="url" placeholder="https://odd-flower-1a93...workers.dev"
                      value={aUrl} onChange={e=>setAUrl(e.target.value)} required/>
                    <div>
                      <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wider">API Key</label>
                      <input type="password" placeholder="mat-khau-api" required
