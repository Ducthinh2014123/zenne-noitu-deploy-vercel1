...
export default function PubWordbank() {
  ...
  const load = useCallback((q_, page_) => {
    setLoading(true);
    pubApi.wordbank({ q: q_, limit: LIMIT, offset: page_ * LIMIT })
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 mb-6">
            {data.map((w, i) => (
              <div key={i}
                className="px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-center hover:border-indigo-500 transition-colors">
                <div className="font-semibold text-white">{w.word}</div>
                <div className="text-xs text-gray-600 mt-0.5">
                  {w.added_at ? new Date(w.added_at).toLocaleDateString('vi-VN') : ''}
                </div>
              </div>
            ))}
            {data.length === 0 && (
