...
export default function PlayIndex() {
  ...
  const create = async () => {
    ...
    try {
      const r = await pubApi.createRoom({ lang, time_limit: Number(timeLimit) });
  ...
  const join = (e) => {
    ...
    if (id) router.push(`/pub/play/${id}`);
  };

  return (
    <PubLayout title="🎮 Chơi Nối Từ Online">
      <p className="text-gray-400 mb-8">
        Tạo phòng và chia sẻ link cho bạn bè — hoặc dùng lệnh <code className="bg-gray-800 px-1.5 py-0.5 rounded text-indigo-300">!taoroom</code> trong Discord.
      </p>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1">⏱️ Giây mỗi lượt: <strong className="text-white">{timeLimit}s</strong></label>
              <input
              ...
              />
              <div className="flex justify-between text-xs text-gray-600 mt-1"><span>10s</span><span>120s</span></div>
            </div>
            <button
              onClick={create}
              disabled={loading}
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-6">
          ...
          <form onSubmit={join} className="space-y-4">
            <div>
              ...
              <input
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 font-mono text-lg tracking-widest uppercase"
                placeholder="abc123"
                value={joinId}
                maxLength={6}
          ...
          <p className="mt-4 text-xs text-gray-600">
            Lấy Room ID từ link mời hoặc từ lệnh <code className="text-indigo-400">!taoroom</code> trong Discord.
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 bg-gray-900 border border-gray-800 rounded-xl p-6">
        ...
        <ol className="space-y-2 text-sm text-gray-400">
          <li>🔢 Host tạo phòng và chia sẻ link cho mọi người.</li>
          <li>👤 Mọi người nhập tên và vào phòng.</li>
          <li>▶️ Host nhấn <strong>Bắt đầu</strong> khi đủ người.</li>
          <li>🔤 Mỗi lượt, nhập từ bắt đầu bằng chữ cuối của từ trước.</li>
          <li>❤️ Mỗi người có 3 mạng. Hết mạng = bị loại.</li>
          <li>🏆 Người còn lại cuối cùng hoặc nhiều điểm nhất thắng!</li>
        </ol>
      </div>
    </PubLayout>
  );
