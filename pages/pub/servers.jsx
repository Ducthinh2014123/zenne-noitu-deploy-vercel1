  ...
  return (
    <PubLayout title="🌐 Servers">
      ...
      <div className="mb-6 flex items-center gap-3">
        <span className="text-3xl font-bold text-indigo-400">{data.total.toLocaleString()}</span>
        <span className="text-gray-400">servers đang sử dụng bot</span>
      </div>
          ...
          {data.data.map(g => (
            <div key={g.id} className="flex items-center gap-3 p-4 bg-gray-900 border border-gray-800 rounded-xl hover:border-indigo-500/50 transition-colors">
              {g.icon ? (
                ...
                <div className="w-12 h-12 rounded-full bg-indigo-900 flex items-center justify-center text-xl flex-shrink-0">
                  🐟
                </div>
              )}
              <div className="min-w-0">
                <div className="font-semibold text-white truncate">{g.name}</div>
                <div className="text-sm text-gray-400">👥 {g.member_count?.toLocaleString()} thành viên</div>
              </div>
            </div>
          ))}
          {data.data.length === 0 && (
            <div className="col-span-full text-center py-12 text-gray-500">Chưa có server nào.</div>
          )}
        </div>
      )}
      {data.total > 50 && (
        <p className="mt-4 text-center text-xs text-gray-600">Hiển thị 50 server lớn nhất trong tổng số {data.total}.</p>
      )}
    </PubLayout>
  );
}
